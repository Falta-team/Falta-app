...
class _SplashScreenState extends State<SplashScreen>
  ...
  void initState() {
    ...
    _strokeFadeAnim = Tween<double>(begin: 1.0, end: 0.0).animate(
  ...
  Future<void> _runSequence() async {
    ...
    // 2 — fill appears, stroke fades (0.7s)
    await _fillController.forward();
    // 3 — red dots pop in (0.6s)
...
class FaltaLogoPainter extends CustomPainter {
  ...
  void _drawDots(Canvas canvas, Paint paint) {
    ...
    for (final d in dots) {
      final cx = d[0]; final cy = d[1];
      final rx = d[2]; final ry = d[3];
      canvas.save();
